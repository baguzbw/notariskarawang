...
import {
  ArrowRight, Briefcase, Calendar, ClipboardList,
  Clock, FileText, GitBranch, Mail, MapPin,
  Phone, PlusCircle, Settings, TrendingUp,
} from "lucide-react";
...
export default async function AdminDashboardPage() {
  const [
    ...
    supabase.from("artikel_hukum").select("id, judul, created_at").order("created_at", { ascending: false }).limit(5),
    supabase.from("info_kegiatan").select("id, judul, tanggal_kegiatan").order("created_at", { ascending: false }).limit(3),
    getProfilKantor(),
  ]);
    ...
    <div className="space-y-8">
      ...
      <div className="bg-dongker text-cream p-8 relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <h1 className="text-3xl font-semibold text-cream mb-1">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
          {stats.map(({ label, total, icon: Icon, href, add }) => (
            <div
              key={label}
              className="bg-cream-light border border-cream-dark p-5 group relative overflow-hidden hover:border-dongker/40 transition-colors"
            >
              <span className="absolute -bottom-2 -right-1 font-bold text-7xl text-dongker/[0.04] select-none leading-none">
                {total}
              </span>
              <div className="flex items-start justify-between mb-4">
                ...
                <Link
                  href={add}
                  className="text-dongker/30 hover:text-dongker transition-colors"
                  title={`Tambah ${label}`}
                >
                  <PlusCircle size={14} />
                </Link>
              ...
              <Link
                href={href}
                className="text-xs text-dongker/50 hover:text-dongker transition-colors inline-flex items-center gap-1 group-hover:gap-1.5 relative z-10"
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-cream-light border border-cream-dark">
          <div className="flex items-center justify-between px-6 py-4 border-b border-cream-dark">
            ...
            <Link
              href="/admin/artikel"
              className="text-xs text-dongker/40 hover:text-dongker transition-colors inline-flex items-center gap-1"
            >
          ...
          <div className="divide-y divide-cream-dark">
            {recentArtikel && recentArtikel.length > 0 ? (
              recentArtikel.map((a) => (
                <div
                  key={a.id}
                  className="flex items-center justify-between px-6 py-3.5 hover:bg-cream-dark/20 transition-colors"
                >
                  <span className="text-sm text-dongker font-medium truncate">{a.judul}</span>
                  <span className="text-xs text-dongker/30 shrink-0 ml-4 hidden sm:block">
        ...
        <div className="space-y-4">
            ...
            <div className="divide-y divide-cream-dark">
              {recentKegiatan && recentKegiatan.length > 0 ? (
                recentKegiatan.map((k) => (
                  <div key={k.id} className="px-5 py-3 hover:bg-cream-dark/20 transition-colors">
          ...
          <div className="bg-cream-light border border-cream-dark">
            ...
            <div className="px-5 py-4 space-y-3">
              ...
              {(profil?.telepon || profil?.hp) && (
                <div className="flex items-center gap-2.5">
                  ...
                  <p className="text-xs text-dongker/60">
                    {profil?.telepon}{profil?.telepon && profil?.hp ? " / " : ""}{profil?.hp}
                  </p>
                </div>
              )}
              {profil?.email && (
              ...
              {!profil?.alamat && !profil?.telepon && !profil?.email && (
                <p className="text-xs text-dongker/30">Data kantor belum diisi.</p>
              )}
            </div>
            <div className="px-5 pb-5">
              <Link
                href="/admin/data-kantor"
