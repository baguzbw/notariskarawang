...
import { ArrowRight, Award, HeartHandshake, Mail, Phone, ShieldCheck, Zap } from "lucide-react";
...
export default async function HomePage() {
  const [profil, { data: layananPreview }] = await Promise.all([getProfilKantor(), supabase.from("layanan").select("id, nama, deskripsi").order("urutan", { ascending: true }).limit(4)]);
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 md:py-36 relative">
            <div className="max-w-3xl">
              ...
              <h1 className="text-5xl md:text-6xl font-semibold tracking-tight leading-[1.15] text-cream">
                ...
                <span className="block mt-2">{nama.split(" ").slice(2).join(" ")}</span>
              </h1>
              <p className="text-gold text-sm md:text-base mt-4 font-semibold tracking-[0.2em] uppercase">{gelar}</p>
              <div className="w-16 h-0.5 bg-gold mt-4 mb-8" />
              <p className="text-cream/70 text-lg leading-relaxed mb-10 max-w-xl">Melayani kebutuhan kenotariatan dan pertanahan masyarakat Karawang dengan standar hukum yang akurat, transparan, dan terpercaya.</p>
            ...
            {layananPreview && layananPreview.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {layananPreview.map((item, index) => (
                  <div key={item.id} className="relative bg-cream-light border border-cream-dark p-6 hover:shadow-lg hover:-translate-y-1 transition-all duration-200 group overflow-hidden">
                    <span className="absolute -top-3 -right-1 font-serif text-8xl font-bold text-dongker/[0.06] select-none leading-none group-hover:text-gold/10 transition-colors duration-300">{String(index + 1).padStart(2, "0")}</span>
                    <div className="w-8 h-0.5 bg-gold mb-5" />
                    <h3 className="text-base font-semibold text-dongker mb-2 relative z-10">{item.nama}</h3>
                    <p className="text-sm text-dongker/60 leading-relaxed relative z-10">{item.deskripsi}</p>
                  </div>
                ))}
              </div>
            ) : (
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <p className="text-cream/60 leading-relaxed mb-8">Kantor Notaris & PPAT {nama} telah dipercaya melayani masyarakat Karawang dalam berbagai kebutuhan hukum pertanahan dan kenotariatan.</p>
